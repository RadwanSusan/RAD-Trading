...
import numpy as np
import time
from RAD_trading import send_market_order, close_all_positions, get_positions
...
class TradingBot:
    def __init__(
        self, symbols, timeframe, strategy_class, risk_percentage=1, max_open_trades=3
    ):
        self.symbols = symbols
        self.timeframe = timeframe
        self.strategies = {
            symbol: strategy_class(symbol, timeframe) for symbol in symbols
        }
    ...
    def calculate_position_size(self, entry_price, stop_loss):
        ...
        position_size = calculate_position_size(
            balance, self.risk_percentage, entry_price, stop_loss
        )
    ...
    def execute_trades(self, signals):
        ...
        for symbol, signal in signals.items():
            if len(positions) >= self.max_open_trades:
                self.logger.info(
                    f"Maximum number of open trades ({self.max_open_trades}) reached. Skipping trade execution for {symbol}."
                )
                continue
            ...
            if signal in ["buy", "sell"]:
                ...
                tick_volume = self.calculate_position_size(
                    symbol, entry_price, stop_loss
                )

                self.logger.info(
                    f"Opening {signal} position for {symbol} with tick_volume {tick_volume}"
                )
                send_market_order(symbol, tick_volume, signal, sl=stop_loss)
    ...
    def run(self):
        ...
        while self.is_running:
            try:
                ...
                signals = {
                    symbol: self.strategies[symbol]
                    .generate_signal(data[symbol])
                    .iloc[-1]["signal"]
                    for symbol in self.symbols
                }
