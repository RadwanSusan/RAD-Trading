...
class Backtester:
    def __init__(self):
        ...
        self.trades = pd.DataFrame(
            columns=['state', 'symbol', 'order_type', 'volume', 'open_time', 'open_price', 'close_time',
                     'close_price', 'sl', 'tp', 'profit', 'commission', 'swap', 'info'])
    ...
    def run_backtest(self):
        ...
        for i in self.ohlc_data.index:
            ...
            for order in orders.orders:
                if order['action'] == 'entry':
                    entry_price = self._apply_slippage(data['open'], order['order_type'])
                    self.trades.loc[len(self.trades), self.trades.columns] = [
                        'open', order['symbol'], order['order_type'], order['volume'],
                        data['time'], entry_price, '', '', order['sl'], order['tp'], 0, 0, 0, order['info']
                    ]
                    ...
                    self.trades.loc[order['trade_id'], [
            ...
            for x in open_trades.index:
                ...
                if t['order_type'] == 'buy':
                    ...
                    if t['sl'] <= data['high'] and t['sl'] != 0:
                        ...
                        current_balance += profit + commission + swap
            self.equity_curve = self.equity_curve.append({'time': data['time'], 'equity': current_balance}, ignore_index=True)
        ...
        for x in open_trades.index:
            ...
            current_balance += profit + commission + swap
        self.trades['profit_net'] = self.trades['profit'] + self.trades['commission'] + self.trades['swap']
    ...
    def _apply_slippage(self, price, order_type):
        slippage_factor = 1 + self.slippage if order_type == 'buy' else 1 - self.slippage
        return price * slippage_factor
    def _calculate_profit(self, trade, exit_price):
        if trade['order_type'] == 'buy':
            return ((exit_price - trade['open_price']) * trade['volume']) * self.exchange_rate
        elif trade['order_type'] == 'sell':
            return ((trade['open_price'] - exit_price) * trade['volume']) * self.exchange_rate
    def _calculate_commission(self, trade):
        return self.commission * trade['volume']
    def _calculate_swap(self, trade, close_time):
        ...
        swap = swap_rate * days_held * trade['volume']
        # Apply triple swap
        triple_swap_days = days_held // 7
        swap += 2 * swap_rate * triple_swap_days * trade['volume']
        return swap
    def visualize_backtest(self, indicators=[], num_trades=None):
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03, subplot_titles=('Price', 'Equity'), row_heights=[0.7, 0.3])
        # Price chart
        fig.add_trace(go.Candlestick(x=self.ohlc_data['time'],
                                     open=self.ohlc_data['open'],
                                     high=self.ohlc_data['high'],
                                     low=self.ohlc_data['low'],
                                     close=self.ohlc_data['close'], name='OHLC Data'), row=1, col=1)
        for indicator in indicators:
            fig.add_trace(go.Scatter(x=self.ohlc_data['time'], y=self.ohlc_data[indicator], name=indicator), row=1, col=1)
        # Equity curve
        fig.add_trace(go.Scatter(x=self.equity_curve['time'], y=self.equity_curve['equity'], name='Equity'), row=2, col=1)
        # Add trade markers
        ...
        return fig
    def plot_pnl(self):
        fig = go.Figure(data=[go.Scatter(x=self.trades['close_time'], y=self.trades['profit_cumulative'], mode='lines')])
        fig.update_layout(title='Cumulative PnL', xaxis_title='Time', yaxis_title='Profit/Loss')
        return fig
    def plot_drawdown(self):
        ...
        fig = go.Figure(data=[go.Scatter(x=self.trades['close_time'], y=drawdowns, mode='lines', fill='tozeroy')])
