...
import numpy as np
def calculate_temporary_impact(order_size, average_daily_volume, volatility, market_cap):
    """
    Calculate temporary market impact using the Square Root Law.
    :param order_size: Size of the order in shares
    :param average_daily_volume: Average daily trading volume of the stock
    ...
    # Calculate impact
    impact = beta * volatility * np.power(relative_order_size, gamma) * np.power(market_cap / 1e9, -0.25)
    return impact
