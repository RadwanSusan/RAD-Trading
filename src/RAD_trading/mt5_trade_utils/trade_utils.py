...
def get_positions(magic=None):
    if mt5.positions_total() > 0:
        ...
        return pd.DataFrame()
def place_pending_order(symbol, volume, order_type, price, sl=0.0, tp=0.0,
                        deviation=20, comment='', magic=0, type_filling=mt5.ORDER_FILLING_IOC):
    order_type_dict = {
...
def calculate_margin_required(symbol, volume, order_type):
    symbol_info = mt5.symbol_info(symbol)
    if symbol_info is None:
        print(f"Failed to get symbol info for {symbol}")
        return None
    order_type_dict = {'buy': mt5.ORDER_TYPE_BUY, 'sell': mt5.ORDER_TYPE_SELL}
    margin = mt5.order_calc_margin(order_type_dict[order_type], symbol, volume, symbol_info.ask)
    if margin is None:
...
def wait_for_market_open(symbol, timeout=3600):
    ...
    while time.time() - start_time < timeout:
        symbol_info = mt5.symbol_info(symbol)
        if symbol_info is not None and symbol_info.trade_mode != mt5.SYMBOL_TRADE_MODE_DISABLED:
            return True
        time.sleep(1)
...
def get_exposure(symbol):
    ...
    if positions is None:
        return 0
    exposure = sum(pos.volume if pos.type == mt5.ORDER_TYPE_BUY else -pos.volume for pos in positions)
    return exposure
